...
from .utkonos_utils import set_city, get_product_links, parse_product

def get_all_data(shop_name):
    driver = get_browser("UTKONOS")
    all_data = []

    try:
        for actual_city_name in cities:
            print(f"[{shop_name}] Инициализация сессии (обход Qrator) для: {actual_city_name}")
            
            status_code = set_city(driver, actual_city_name, shop_name)
            
            if status_code == 999:
                print(f"[{shop_name}] Ошибка 999. Город не найден.")
                continue
