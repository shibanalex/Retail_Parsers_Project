...
from .samokat_utils import set_city, get_product_links, parse_product

def get_all_data(shop_name):
    driver = get_browser("SAMOKAT")
    all_data = []

    try:
        for actual_city_name in cities:
            print(f"[{shop_name}] Поиск города (выбор адреса): {actual_city_name}")
