...
def init_driver(parser_cfg_name="", headless=False):
    ...
    # Вычисляем пути с учетом папки cataloged_pkg
    base_dir = os.path.dirname(os.path.abspath(__file__)) # cataloged_pkg
    parser_root = os.path.dirname(base_dir) # папка парсера cataloged
    global_root = os.path.dirname(parser_root) # корень всех парсеров
    
    cfg_path = os.path.join(global_root, "parsers_core", "share_parsers.cfg")
