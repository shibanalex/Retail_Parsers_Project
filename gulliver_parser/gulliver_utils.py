...
def set_city(driver, city_name, shop_name):
    global _cached_catalog_products, _current_city_shops
    _cached_catalog_products = {}
    _current_city_shops = []
    ...
    graphql_query_shops = """
    query shopsInGeoPolygon($corners: [GeoPointInput]!, $useZone: Boolean, $customerType: String) {
    ...
    payload_shops = {
        ...
        "query": graphql_query_shops
    }
    
    try:
        resp = session.post(url, json=payload_shops, timeout=10)
        if resp.status_code != 200:
            print(f"[{shop_name}] Ошибка API (HTTP {resp.status_code}) при запросе магазинов.")
            return 500
        ...
        if _current_city_shops:
            print(f"[{shop_name}] В городе {city_name} найдено активных магазинов: {len(_current_city_shops)}")
            return 200
        else:
            return 999
            
    except Exception as e:
        print(f"[{shop_name}] Ошибка загрузки списка магазинов: {e}")
        return 500
...
def get_product_links(driver, query, shop_name):
    ...
    for shop in _current_city_shops:
        ...
        try:
            while collected_in_shop < MAX_PRODUCTS_PER_QUERY:
                payload = {
                ...
                for item in edges:
                    ...
                    _cached_catalog_products[cache_key] = {
                        ...
                        "Вес": weight,
                        "Адрес": shop_address,
                        "Оригинальный URL": prod_url
                    }
                    
                    if cache_key not in links:
                        links.append(cache_key)
                        collected_in_shop += 1
                        
                    if collected_in_shop >= MAX_PRODUCTS_PER_QUERY:
                        break
